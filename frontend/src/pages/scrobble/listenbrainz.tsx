...
type MatchTag = "matched" | "duplicate";
type SortKey = "title" | "artist" | "time" | "status";
...
function AlbumArt({
  ...
  if (!src || err) {
    return (
      <div
        className="rounded-md bg-gray-100 dark:bg-gray-800 flex items-center justify-center flex-shrink-0"
...
function TagBadge({ tag }: { tag: MatchTag }) {
  return tag === "duplicate" ? (
    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-amber-100 text-amber-700 dark:bg-amber-400/15 dark:text-amber-400">
      <span className="w-1.5 h-1.5 rounded-full bg-amber-500 dark:bg-amber-400" />
      Duplicate
    </span>
  ) : (
    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-green-100 text-green-700 dark:bg-green-400/15 dark:text-green-400">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {(["title", "artist", "time", "status"] as SortKey[]).map((k) => (
              <SortButton
                key={k}
...
type MatchedFilter = "all" | MatchTag;

function MatchedTable({ entries }: { entries: MatchedEntry[] }) {
  const [filter, setFilter] = useState<MatchedFilter>("all");
  ...
  const matchedCount = entries.filter((e) => e.tag === "matched").length;
  const dupCount = entries.filter((e) => e.tag === "duplicate").length;

  const filtered = entries.filter((e) => filter === "all" || e.tag === filter);
  const sorted = [...filtered].sort((a, b) => {
    if (sort === "title") return a.song_name.localeCompare(b.song_name);
    if (sort === "artist") return a.artist.localeCompare(b.artist);
    if (sort === "status") return a.tag.localeCompare(b.tag);
    return (
  ...
  return (
    <div className="flex flex-col gap-4">
      <TableControlRow
        filterEl={
          <SubFilterPills<MatchedFilter>
            value={filter}
            onChange={setFilter}
            options={[
              { value: "all", label: `All (${entries.length})` },
              { value: "matched", label: `New Entries (${matchedCount})` },
            ...
            {displayed.length === 0 ? (
              ...
              displayed.map((entry, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 max-w-[160px] sm:max-w-none">
                    <div className="flex items-center gap-3">
                      <div className="w-[38px] h-[38px] rounded-md bg-gray-100 dark:bg-gray-800 flex items-center justify-center flex-shrink-0">
                        <svg
                          width="13"
                          height="13"
                          viewBox="0 0 24 24"
                      ...
                      <div className="min-w-0 flex-1">
                        <p
                          className="text-sm text-gray-600 dark:text-gray-400 truncate leading-tight"
    ...
    getListenbrainzLog()
      .then((logData) => {
        ...
        logData.forEach((entry) => {
          if (entry.tag === "matched" || entry.tag === "duplicate") {
            matched.push({
              id: String(entry.id),
              song_name: entry.title || "Unknown Title",
              artist: entry.artist || "Unknown Artist",
              album: entry.album || "Unknown Album",
              album_art_url: getCoverArtUrl(entry.song_id),
              listened_at: entry.timestamp,
              tag: entry.tag as MatchTag,
            });
          } else if (entry.tag === "unmatched") {
            unmatched.push({
              id: String(entry.id),
      ...
      <div className="flex flex-col gap-5">
        <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="p-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              {
                label: "Total Fetched",
                value: loading ? "—" : matchedCount + unmatchedCount,
                color: undefined,
              },
              {
                label: "New Entries",
                value: loading ? "—" : matchedEntries,
                color: "#7F77DD",
              },
              {
                label: "Unmatched",
              ...
              <div
                key={c.label}
                className="rounded-xl bg-gray-50 dark:bg-gray-800/70 border border-gray-100 dark:border-gray-700/50 p-5"
              >
                <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2">
                  {c.label}
                </p>
                <p
                  className="text-3xl font-semibold tabular-nums text-gray-800 dark:text-white/90"
        ...
        <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100 dark:border-gray-800 flex-wrap gap-3">
            <div>
              <h4 className="text-sm font-semibold text-gray-800 dark:text-white/90">
                {activeTab === "matched" ? "Matched Songs" : "Unmatched Songs"}
              </h4>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                {activeTab === "matched"
                  ? `${matchedEntries} matched · ${matchedCount - matchedEntries} duplicates`
                  : `${unmatchedCount} unidentified`}
              </p>
            </div>

            <div className="flex items-center rounded-xl bg-gray-100 dark:bg-gray-800 p-1 gap-0.5">
              {(["matched", "unmatched"] as ActiveTab[]).map((tab) => (
                <button
                  key={tab}
                ...
                >
                  <span
                    ...
                    style={{
                      backgroundColor:
                        tab === "matched" ? "#639922" : "#E24B4A",
                    }}
                  />
                  {tab === "matched" ? "Matched" : "Unmatched"}
                  {!loading && (
                    <span className="ml-0.5 text-[10px] tabular-nums text-gray-400 dark:text-gray-500">
                      {tab === "matched" ? matchedCount : unmatchedCount}
                    </span>
                  )}
                </button>
              ))}
          ...
          <div className="p-6">
            {loading ? (
              ...
              <MatchedTable entries={data?.matched ?? []} />
            ) : (
              <UnmatchedTable
